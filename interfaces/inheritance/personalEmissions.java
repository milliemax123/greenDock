 ...
 *x23471092
 * this extends emission\Records and inherits totalco2, emission summary(0, and emission category()
 */
...
public class personalEmissions extends emissionRecords implements Serializable {
    ...
    // a simple method that roughly calculates a users amount of co2 in tonnes per year
    public double calculatorResult(){ 
        //this is all in kg 
        double total = 0;
        //car
        total = total +(carKm * 0.00021 *52); //0.21 kg = a 1km trip so each km times that per week then times 52 is equal to years worth of car trips      
        //public transport
        total = total +(busTrips * 0.0015 * 52);//rough guess of 1.5kg worth of co2 for public transport every week by a year        
        //flighst
        total = total + (flights * 0.25);      
        //orgininally was asiing people for their kwt electric but no one would know that so now its house size and heating type 
         total = total + (houseSize *0.50);
    ...
    @Override
    public String toString() { //use dto print all details
